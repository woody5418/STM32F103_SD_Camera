...
void Test_f_write(void)//写数据到文件，如果没有此文件则创建文件
{
    ...
    FRESULT res;         // FatFs function common result code
		FIL Make_file;
    char file_name[20]="新建文件.txt";
    char Storage_buffer[] ="this is a test!";
...
//unsigned
void readfirenum(char *path,char *data)
{
	//char data[20];
	FATFS fs;            // Work area (file system object) for logical drive
	FIL file;            // file objects
	...
	f_open(&file, path, FA_OPEN_EXISTING | FA_READ);//打开存在的文件，如果没有则返回错误

//	res = f_lseek(&file,ofs); //指针移到文件ofs个字节处

	f_read(&file, data, file.fsize, &br);
	f_close(&file);
