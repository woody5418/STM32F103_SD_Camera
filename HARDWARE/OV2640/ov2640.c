...
u8 OV2640_Init(void)
{
    ...
    // OV_PCLK C4 替换为B8
    GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_8; 	//PB4 输入 上拉
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;  // 上拉输入
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOB,&GPIO_InitStructure);
    GPIO_SetBits(GPIOB,GPIO_Pin_8);
    // OV_PWDN  B3 替换为B9
    GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_9; 	//PB3 输出
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_Init(GPIOB,&GPIO_InitStructure);
    GPIO_SetBits(GPIOB,GPIO_Pin_9);

    //数据口PC0-7
    GPIO_InitStructure.GPIO_Pin  = 0xff; //PB0~7 输入 上拉
    ...
    //HREF替换为PA1
    GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_1; //PG14输入
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
    GPIO_Init(GPIOA, &GPIO_InitStructure);
    GPIO_SetBits(GPIOA,GPIO_Pin_1);

    //RESET 替换为PA2
    GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_2; //PG15输出
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_Init(GPIOA, &GPIO_InitStructure);
